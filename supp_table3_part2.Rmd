---
title: "Quantile normalization of phenotypes"
...
covariates<-subset_Mexican_pheno[c(1:6)]
```
...
```{r}
data_mat <- data.matrix(subset_Mexican_pheno[,7:75])
...
```{r}
fit <- lapply(covariates_norm_table[,7:75], function(x) lm(x ~ AI + birth_year + GENDER + log(WEIGHT_FINAL_NORM_OVERALL) + CENTER, data = covariates_norm_table,na.action = na.omit))
```
...
for (i in 1:length(fit))
{
  ...
  #coef_table
  coef_table_vals <- coef_table[which(rownames(coef_table) %in% c("AI","birth_year","GENDER","log(WEIGHT_FINAL_NORM_OVERALL)","CENTERC","CENTERM","CENTERS")),
...
library(xlsx)
write.table(pheno_results,"/Users/melissaspear/Dropbox/HCHS_SOL/complex_traits/extended_supplementary_table3_new_090720_quantile_normalized.txt",row.names=F,col.names=T,sep='\t',quote=F)
write.xlsx(pheno_results,"/Users/melissaspear/Dropbox/HCHS_SOL/complex_traits/extended_supplementary_table3_new_part2_090720.xlsx")
```
