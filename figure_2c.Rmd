...
#
```{r}
#pdf("/Users/melissaspear/Dropbox/HCHS_SOL/paper_figures/Mexicans_ROH_All_NAM_short_medium_tracts_072619.pdf",height=2.25,width=2.375,pointsize=10)
...
mtext(side=2,"Proportion of genome with ROH",line=1.0,cex.lab=0.8)
#legend("bottomright",legend=c("All ROH","All NAM ROH","All Short ROH","All Short NAM ROH","All Medium ROH","All Medium NAM ROH"),col=c("black","black","#D95F02","#D95F02","#7570B3","#7570B3"),lty=c(1,3,1,3,1,3),cex=0.5,bg="white")
#CEU all ROH 
...
#CEU medium ROH 
lw3 <- loess(log(ROH_B_Mb) ~ estimated_birth_year,data=roh_with_pheno)
j3 <- order(roh_with_pheno$estimated_birth_year)
lines(roh_with_pheno$estimated_birth_year[j3],lw3$fitted[j3],col="#7570B3",lwd=1)
#AI medium roh
lw3_ai <- loess(log(ROH_B_Mb) ~ estimated_birth_year,data=roh_with_pheno_nam)
