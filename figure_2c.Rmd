...
nam_roh_calls<-read.table("/Volumes/LaCie/Downloads/split_roh_files/NAM_ROH_segments/reclassified_classes/summed_calls2/HCHS_SOL_Mexicans_alldecades_summed_NAM_ROH_calls_newclasses_080519.txt",header=F,sep='\t')

#adding column titles 
names(nam_roh_calls)=c("IID","ROH_A","ROH_B","ROH_C")

#New column of bp to Mbp
#nam_roh_calls$ROH_A_Mb = nam_roh_calls$ROH_A/1000000
#nam_roh_calls$ROH_B_Mb = nam_roh_calls$ROH_B/1000000
#nam_roh_calls$ROH_C_Mb = nam_roh_calls$ROH_C/1000000
...
roh_with_pheno<-merge(nam_roh_calls,pheno_subsetted,by.x=("IID"),by.y=c("V2"))
```
...
roh_with_pheno_admixture<-merge(roh_with_pheno,fam_file_with_IDs, by.x=c("IID"),by.y=c("IID"))

#Calcualting AI genome length 
roh_with_pheno_admixture$AI_both_haps_length<- (roh_with_pheno_admixture$AI * 5563642548)
```

```{r}
#creating new variable summing three ROH columns 
roh_with_pheno_admixture$ROH_allclasses_sums<- rowSums(roh_with_pheno_admixture[,c("ROH_A", "ROH_B","ROH_C")])

#normalizing ROH by global ancestry 
roh_with_pheno_admixture$ROH_allclasses_sums_NORM<-(roh_with_pheno_admixture$ROH_allclasses_sums)/((roh_with_pheno_admixture$AI_both_haps_length)/(2)) 

#proportion of genome covered in ROH 
roh_with_pheno_admixture$prop_genome_AI_ROH<-(roh_with_pheno_admixture$ROH_allclasses_sums_NORM * 2781821274)
```
